...
export default function FileUploader() {
  ...
  const [uploadedImages, setUploadedImages] = useState<File[]>([]); // Updated to store File objects
  ...
  const handleImageUpload = async (e) => {
    ...
    uploadedImages.forEach((image, index) => {
      formData.append(`semester_photo${index + 1}`, image); // Adjust field name dynamically
    });

    try {
      const response = await axios.post(
        'http://127.0.0.1:8003/api/v1/student/semester/experience',
        formData,
        {
          headers: {
      ...
      toast({
        ...
        status: 'error',
        duration: 5000,
        description: error.response.data || 'Image upload failed, please try again',
      });
    } finally {
      setIsloading(false);
      onClose();
